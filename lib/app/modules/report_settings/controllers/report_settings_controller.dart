...
class ReportSettingsController extends GetxController {
   var is_loading=false.obs;
  var model_appsettings=Rxn<ModelReportSettings>();
  Map<String, dynamic>? modelJson; 
   var toogle_isselected = 0.obs;
   var is_moduletile_slct=false.obs;
   TextEditingController txt_fromdate_controller = TextEditingController(
          text: DateFormat('dd-MMM-yyyy').format(DateTime.now()));
   TextEditingController txt_todate_controller = TextEditingController(
          text: DateFormat('dd-MMM-yyyy').format(DateTime.now()));
   var Slct_fromdate = DateTime.now().obs;
   var Slct_todate = DateTime.now().obs;
     RoundedLoadingButtonController btn_apptheme_controller =
      RoundedLoadingButtonController();
      final format = DateFormat("hh:mm a");
     TimeOfDay? time;
     Rxn<Currency> dropdown_Currency = Rxn();
  ...
  Getdata_report_settings()async{
    try {
      ...
      var responce = await File("assets/json/Ownerapp/model_report_settings.json").readAsString();
